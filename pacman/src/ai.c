...
move_t GraphSearch(struct heap *Graph, node_t* parent, propagation_t propagation, int budget){
    ...
    prop_t *propagationTotal[4];
    for(int i = 0; i < 4; i++){
        propagationTotal[i]->total = 0;
        propagationTotal[i]->numChild = 0;
    }
    ...
    while(frontier.count != 0 && budget > Graph->count){
        ...
        for(int i = 0; i < 4; i++){
            ...
            if(applyAction(currNode, &nextNode, i)){
                ...
                backPropagation(propagationTotal, propagation, nextNode, nextNode->acc_reward);
                
                if(nextNode->state.Lives < currNode->state.Lives){
                    free(nextNode);
                }
                else{
...
void backPropagation(prop_t *propagationTotal[], propagation_t propagation, node_t *node, float accReward){
    
    if(node->parent->parent == NULL){
        move_t move = node->parent->move;
        if(propagation){
//            int total = propagationTotal[move]->total * propagationTotal[move]->numChild;
  ///          int newTotal = total + accReward;
     //       propagationTotal[move]->numChild += 1;
       //     int newAve = newTotal / propagationTotal[move]->numChild;
        }
        else{
            FILE* fil = fopen("Debug.txt","a");
            fprintf(fil,"prop %f\n",propagationTotal[0]->total);
            fclose(fil);
           // if(propagationTotal[move]->total < accReward){
         //       propagationTotal[move]->total = accReward;
           //     propagationTotal[move]->numChild += 1;
          //  }
        }
    }
    else{
        backPropagation(propagationTotal, propagation, node->parent, accReward);
    }
}
