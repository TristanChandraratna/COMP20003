...
void heap_display(struct heap* h) {
	int i;
    FILE* fil = fopen("HeapDisplay.txt","a");
    fprintf(fil,"%d\n",h->count);
	for(i=0; i<h->count; ++i) {
	    node_t* n = h->heaparr[i];
	    fprintf(fil,"priority = %d,depth = %d, reward = %f,move = %d, loc (%d,%d)\n", n->priority, n->depth, n->acc_reward, n->move,n->state.Loc[4][0],n->state.Loc[4][1]);
